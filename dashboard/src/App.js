...
function SimMessage({ message, onAnnotate, onRate, onRewrite }) {
  ...
  const renderText = () => {
    ...
    for (const a of savedAnnotations) {
      const idx = remaining.indexOf(a.text);
      if (idx >= 0) {
        if (idx > 0) parts.push({ text: remaining.substring(0, idx), highlight: false });
        parts.push({ text: a.text, highlight: true, rating: a.rating });
        remaining = remaining.substring(idx + a.text.length);
      }
    }
    if (remaining) parts.push({ text: remaining, highlight: false });
    return parts.map((p, i) => p.highlight ? (
  ...
  const simRewrite = async (originalText, rewrittenText) => {
    try {
      ...
      setSimThread(prev => prev.map(m =>
        (m.sender === 'us' && m.text === originalText)
          ? { ...m, text: rewrittenText, original: m.original || originalText }
          : m
      ));
