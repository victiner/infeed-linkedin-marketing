...
function AssetsPanel() {
  ...
  const [editing, setEditing] = useState(null); // { category, id, name, url, description, segment }
...
function AvatarsPanel() {
  ...
  if (selected) {
    ...
    const ExampleRow = ({ ex }) => {
      ...
      return (
        <div style={{ background: '#FFFFFF', border: `1px solid ${ex.isCanonical ? '#E8734A' : 'rgba(0,0,0,0.08)'}`, borderLeft: `3px solid ${valenceColor}`, borderRadius: 10, padding: 12, marginBottom: 8 }}>
    ...
    const AddForm = ({ valence }) => {
      const isOpen = adding?.valence === valence;
      if (!isOpen) {
        return (
          <button onClick={() => startAdd(valence)}
      ...
      return (
        <div style={{ background: '#FFFFFF', border: `1px solid ${valence === 'good' ? '#C7DDA8' : '#F5C2C2'}`, borderRadius: 10, padding: 14, marginBottom: 10 }}>
          <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
            {typeOptions.map(o => (
    ...
    return (
      <div style={{ padding: '20px 24px', overflow: 'auto', height: '100%', background: '#FAF9F7' }}>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          {goodExamples.map(ex => <ExampleRow key={ex.airtableId} ex={ex} />)}
        </div>
        ...
        <div>
          ...
          {badExamples.map(ex => <ExampleRow key={ex.airtableId} ex={ex} />)}
        </div>
      </div>
    );
  }
