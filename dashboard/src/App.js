...
function AvatarsPanel() {
  ...
  const openCell = async (avatarId) => {
    setSelected(avatarId); setExamples(null); setExamplesLoading(true);
    try {
      const res = await api.getAvatarExamples(avatarId);
  ...
  const toggleCanonical = async (ex) => {
    try {
      ...
      setExamples(prev => prev.map(e => e.airtableId === ex.airtableId ? { ...e, isCanonical: !e.isCanonical } : e));
      loadCoverage();
    } catch (e) { setError(e.message); }
  };
  ...
  if (selected) {
    ...
    return (
      <div style={{ padding: '20px 24px', overflow: 'auto', height: '100%', background: '#FAF9F7' }}>
        <button onClick={() => { setSelected(null); setExamples(null); }}
        ...
        <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: 18, marginBottom: 14 }}>
          ...
          <div style={{ fontSize: 12, color: '#6B6B6B', marginTop: 6 }}>
            {examplesLoading ? 'Loading examples…' : `${(examples || []).length} examples · ${(examples || []).filter(e => e.isCanonical).length} canonical (★)`}
          </div>
        </div>
        {(examples || []).length === 0 && !examplesLoading && (
          <div style={{ background: '#FFFFFF', border: '1px dashed rgba(0,0,0,0.12)', borderRadius: 12, padding: 32, textAlign: 'center', color: '#6B6B6B', fontSize: 13 }}>
            No examples yet for this avatar. Train a few in <strong>AI Training</strong> with this seniority + stage, then they'll auto-tag here.
          </div>
        )}
        {(examples || []).map(ex => (
          <div key={ex.airtableId} style={{ background: '#FFFFFF', border: `1px solid ${ex.isCanonical ? '#E8734A' : 'rgba(0,0,0,0.08)'}`, borderRadius: 10, padding: 12, marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 11, color: '#9E9E9E', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>{ex.type} · {ex.timestamp ? new Date(ex.timestamp).toLocaleDateString() : ''}</div>
                {ex.original && (
              ...
              <button onClick={() => toggleCanonical(ex)}
                title={ex.isCanonical ? 'Remove canonical' : 'Mark canonical'}
                style={{ fontSize: 18, padding: '4px 10px', border: 'none', borderRadius: 6, background: ex.isCanonical ? '#FDF0EB' : '#FAFAF8', color: ex.isCanonical ? '#E8734A' : '#9E9E9E', cursor: 'pointer' }}>
                {ex.isCanonical ? '★' : '☆'}
              </button>
            </div>
          </div>
      ...
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 14 }}>
        {stageList.map(s => (
          <button key={s} onClick={() => setStage(s)}
            style={{
              fontSize: 12, padding: '6px 12px', border: 'none', borderRadius: 6, cursor: 'pointer',
        ...
        <table style={{ borderCollapse: 'separate', borderSpacing: 4, fontSize: 11, fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}>
          <thead>
            <tr>
              ...
              {situations.map(sit => (
                <th key={sit} style={{ padding: '4px 6px', color: '#6B6B6B', fontWeight: 500, fontSize: 10, textAlign: 'center', minWidth: 70 }}>
                  {sit.replace(/_/g, ' ')}
                </th>
              ))}
            </tr>
