                ...
                return (
                  <div key={i} style={{ maxWidth: '70%', alignSelf: isUs ? 'flex-end' : 'flex-start', marginTop: sameSender ? 1 : 10 }}>
                    ...
                    <div style={{ fontSize: 10, color: '#9E9E9E', marginTop: 3, padding: '0 8px', display: 'flex', justifyContent: isUs ? 'flex-end' : 'flex-start', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ display: 'flex', gap: 2 }}>
                          {[1,2,3,4,5].map(star => (
                            <span key={star} onClick={async (e) => {
                              e.stopPropagation();
                              try {
                                await api.rateMessage({ conversationId: sel.id, leadName: lead?.name, messageText: m.text, rating: star, category: 'tone', wasAutoSent: true });
                                setRatedMessages(prev => ({ ...prev, [msgKey]: star }));
                              } catch {}
                            }} style={{ cursor: 'pointer', fontSize: 11, opacity: 0.5, transition: 'opacity 0.1s' }}
                      ...
                      {isUs && ratedMessages[msgKey] && (
                        <span style={{ fontSize: 10, color: '#E8734A' }}>{'\u2605'} {ratedMessages[msgKey]}/5</span>
                      )}
                    </div>
                  </div>
                );
...
function TrainingPanel() {
  const [trainingType, setTrainingType] = useState('draft');
  ...
  const loadStats = async () => {
    try { const d = await api.getTrainingStats(); setStats(d); } catch {}
  };

  useEffect(() => { loadScenario(); loadStats(); }, []);

  const switchType = (t) => { setTrainingType(t); loadScenario(t); };

  const pickOption = async (text, index) => {
    try {
      await api.recordPreference({ type: scenarioType, scenario, chosen: text, optionIndex: index, isCustom: false });
  ...
  const submitCustom = async () => {
    ...
    try {
      await api.recordPreference({ type: scenarioType, scenario, chosen: customText, optionIndex: -1, isCustom: true });
      setFeedback({ type: 'success', text: 'Recorded' });
      loadStats();
      setTimeout(() => loadScenario(), 600);
    } catch (e) { setFeedback({ type: 'error', text: e.message }); }
  ...
  const TABS = [
    { id: 'draft', label: 'Draft messages', desc: 'Train response style' },
  ...
  return (
    <div style={{ padding: 24, maxWidth: 700, margin: '0 auto', overflowY: 'auto', height: '100%' }}>
      ...
      {loading && <div style={{ padding: 40, textAlign: 'center', color: '#9E9E9E', fontSize: 14 }}>Generating scenario...</div>}

      {/* DRAFT scenarios */}
      {!loading && scenario && scenarioType === 'draft' && (
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {options.map((opt, i) => (
              <button key={i} onClick={() => pickOption(opt, i)}
                style={{ textAlign: 'left', padding: '12px 16px', background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, cursor: 'pointer', fontSize: 13.5, lineHeight: 1.5, color: '#1A1A1A', transition: 'all 0.12s', boxShadow: '0 1px 2px rgba(0,0,0,0.03)', fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}
                onMouseEnter={e => { e.currentTarget.style.borderColor = '#E8734A'; e.currentTarget.style.boxShadow = '0 2px 8px rgba(232,115,74,0.12)'; }}
                onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(0,0,0,0.08)'; e.currentTarget.style.boxShadow = '0 1px 2px rgba(0,0,0,0.03)'; }}>
                {opt}
              </button>
            ))}
          </div>
      ...
      {/* SENTIMENT scenarios */}
      {!loading && scenario && scenarioType === 'sentiment' && (
        <div>
          <div style={{ background: '#FAFAF8', border: '1px solid rgba(0,0,0,0.06)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
      ...
      {/* ROUTING scenarios */}
      {!loading && scenario && scenarioType === 'routing' && (
        <div>
          <div style={{ background: '#FAFAF8', border: '1px solid rgba(0,0,0,0.06)', borderRadius: 12, padding: 20, marginBottom: 20 }}>
