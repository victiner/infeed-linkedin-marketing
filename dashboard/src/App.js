...
function AvatarsPanel() {
  ...
  const startEdit = (ex) => {
    setAdding(null);
    setEditing({ airtableId: ex.airtableId, chosen: ex.chosen || '', original: ex.original || '', feedback: ex.feedback || '' });
  };
  ...
  const saveEdit = async () => {
    ...
    try {
      await api.updateExample(editing.airtableId, {
  ...
  const startAdd = (valence) => {
    setEditing(null);
    const defaultType = valence === 'good' ? 'thumbs_up' : 'thumbs_down';
    setAdding({ valence, type: defaultType, chosen: '', original: '', feedback: '' });
  };
  ...
  if (selected) {
    ...
    const renderExampleRow = (ex) => {
      ...
      const valenceColor = ex.valence === 'good' ? '#3B6D11' : '#A32D2D';
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {!isEditing && (
                <>
                  ...
                  <div style={{ fontSize: 13, color: '#1A1A1A', whiteSpace: 'pre-wrap' }}>{ex.chosen}</div>
                </>
              )}
              {isEditing && (
                <>
                  ...
                  <div style={{ fontSize: 10, color: '#3B6D11', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>{ex.valence === 'good' ? 'GOOD message' : 'BAD message'}</div>
    ...
    const renderAddForm = (valence) => {
      ...
      if (!isOpen) {
        return (
          <button key={`add-${valence}-closed`} onClick={() => startAdd(valence)}
            style={{ fontSize: 12, padding: '8px 14px', border: `1px dashed ${valence === 'good' ? '#C7DDA8' : '#F5C2C2'}`, borderRadius: 8, background: 'transparent', color: valence === 'good' ? '#3B6D11' : '#A32D2D', cursor: 'pointer', marginBottom: 10, fontWeight: 500 }}>
            + Add {valence} example
          </button>
        );
      }
      ...
      return (
        <div key={`add-${valence}-open`} style={{ background: '#FFFFFF', border: `1px solid ${valence === 'good' ? '#C7DDA8' : '#F5C2C2'}`, borderRadius: 10, padding: 14, marginBottom: 10 }}>
          <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>
            {typeOptions.map(o => (
          ...
          <div style={{ fontSize: 10, color: valence === 'good' ? '#3B6D11' : '#A32D2D', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>{valence === 'good' ? 'GOOD message' : 'BAD message'}</div>
          <textarea value={adding.chosen} onChange={e => setAdding(a => ({ ...a, chosen: e.target.value }))}
            placeholder={valence === 'good' ? "Your canonical response for this avatar..." : "An example of what NOT to send for this avatar..."}
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={saveAdd} disabled={busy || !adding.chosen.trim() || (adding.type === 'correction' && !adding.original.trim())}
              style={{ fontSize: 12, padding: '6px 14px', border: 'none', borderRadius: 6, background: '#E8734A', color: '#fff', fontWeight: 500, cursor: busy ? 'wait' : 'pointer', opacity: (!adding.chosen.trim() || (adding.type === 'correction' && !adding.original.trim())) ? 0.5 : 1 }}>
