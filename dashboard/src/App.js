...
function AnnotatableOption({ text, index, verdict, onPick, onRate, onAnnotate }) {
  ...
  const renderText = () => {
    let result = text;
    const highlighted = savedAnnotations.map(a => a.text);
    if (highlighted.length === 0) return result;
    ...
    for (const h of highlighted) {
      ...
      if (idx >= 0) {
        ...
        const ann = savedAnnotations.find(a => a.text === h);
        parts.push({ text: h, highlight: true, rating: ann?.rating });
  ...
  return (
    <div style={{ display: 'flex', gap: 0, position: 'relative' }}>
      <div style={{
        ...
        <button onClick={() => onPick(text, index)} onMouseUp={handleMouseUp}
          style={{
