...
function TrainingPanel() {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 700, margin: '0 auto', overflowY: 'auto', height: '100%' }}>
      ...
      {loading && trainingType !== 'conversation' && <div style={{ padding: 40, textAlign: 'center', color: '#9E9E9E', fontSize: 14 }}>Generating scenario...</div>}
      ...
      {!loading && scenario && scenarioType === 'draft' && (
        <div>
          ...
          <div style={{ fontSize: 11, color: '#9E9E9E', marginBottom: 10 }}>
            Click an option to pick it as your preferred response, or rate each one with <strong>+</strong> (good for this candidate) / <strong>−</strong> (bad for this candidate).
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {options.map((opt, i) => {
              const verdict = ratedOptions[i];
              ...
              return (
                <div key={i} style={{
                  ...
                  <button onClick={() => pickOption(opt, i)}
                    style={{
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', borderLeft: '1px solid rgba(0,0,0,0.06)' }}>
                    <button onClick={(e) => { e.stopPropagation(); rateOption(opt, i, '+'); }} disabled={!!verdict}
                      title="Good for this candidate"
                      style={{
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {convOptions.map((opt, i) => (
                  <button key={i} onClick={() => pickConvOption(opt)}
                    style={{
                      textAlign: 'left', padding: '12px 16px', background: '#FFFFFF',
                      border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, cursor: 'pointer',
                      fontSize: 13.5, lineHeight: 1.5, color: '#1A1A1A',
                      fontFamily: "'IBM Plex Sans', system-ui, sans-serif",
                      boxShadow: '0 1px 2px rgba(0,0,0,0.03)', transition: 'all 0.12s',
                    }}
        ...
        <div>
          ...
          {!targetLoading && targetScenario && (
            <div>
              ...
              <div style={{ fontSize: 12, fontWeight: 500, color: '#6B6B6B', marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.04em' }}>How would you respond?</div>
              <div style={{ fontSize: 11, color: '#9E9E9E', marginBottom: 10 }}>
                Click an option to pick it, or rate with <strong>+</strong> / <strong>−</strong>.
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {targetOptions.map((opt, i) => {
                  const verdict = targetRatedOptions[i];
                  const borderColor = verdict === '+' ? '#639922' : verdict === '-' ? '#A32D2D' : 'rgba(0,0,0,0.08)';
                  return (
                    <div key={i} style={{
                      display: 'flex', gap: 8, alignItems: 'stretch',
                      ...
                      <button onClick={() => pickTargetedOption(opt, i)}
                        style={{
                          flex: 1, textAlign: 'left', padding: '12px 16px', background: 'transparent',
                          border: 'none', cursor: 'pointer', fontSize: 13.5, lineHeight: 1.5, color: '#1A1A1A',
                          fontFamily: "'IBM Plex Sans', system-ui, sans-serif", borderRadius: '12px 0 0 12px',
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', borderLeft: '1px solid rgba(0,0,0,0.06)' }}>
                        <button onClick={(e) => { e.stopPropagation(); rateTargetedOption(opt, i, '+'); }} disabled={!!verdict}
                          title="Good for this type"
                          style={{
                            flex: 1, minWidth: 36, padding: '0 10px', border: 'none',
                            background: verdict === '+' ? '#EAF3DE' : 'transparent',
                            color: verdict === '+' ? '#3B6D11' : '#6B6B6B',
      ...
      {!loading && trainingType !== 'conversation' && (
