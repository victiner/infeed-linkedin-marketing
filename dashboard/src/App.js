...
function AvatarsPanel() {
  ...
  const saveAdd = async () => {
    ...
    try {
      ...
      await api.createExample(selected, payload);
      setAdding(null);
      await refreshCell();
    } catch (e) { setError(e.message); }
    setBusy(false);
  ...
  if (selected) {
    ...
    const renderExampleRow = (ex) => {
      ...
      return (
        <div key={ex.airtableId} style={{ background: '#FFFFFF', border: `1px solid ${ex.isCanonical ? '#E8734A' : 'rgba(0,0,0,0.08)'}`, borderLeft: `3px solid ${valenceColor}`, borderRadius: 10, padding: 12, marginBottom: 8 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
            ...
            {!isEditing && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
                <button onClick={() => toggleCanonical(ex)} title={ex.isCanonical ? 'Remove canonical' : 'Mark canonical'}
                  style={{ fontSize: 16, padding: '4px 10px', border: 'none', borderRadius: 6, background: ex.isCanonical ? '#FDF0EB' : '#FAFAF8', color: ex.isCanonical ? '#E8734A' : '#9E9E9E', cursor: 'pointer' }}>
                  {ex.isCanonical ? '★' : '☆'}
                </button>
                <button onClick={() => startEdit(ex)} style={{ fontSize: 11, padding: '3px 8px', border: '0.5px solid rgba(0,0,0,0.15)', borderRadius: 5, background: '#FFFFFF', color: '#6B6B6B', cursor: 'pointer' }}>Edit</button>
                <button onClick={() => handleDeleteExample(ex)} disabled={busy} style={{ fontSize: 11, padding: '3px 8px', border: '0.5px solid #F5C2C2', borderRadius: 5, background: '#FFFFFF', color: '#A32D2D', cursor: busy ? 'wait' : 'pointer' }}>Delete</button>
              </div>
            )}
          </div>
        </div>
    ...
    return (
      <div style={{ padding: '20px 24px', overflow: 'auto', height: '100%', background: '#FAF9F7' }}>
        ...
        {error && <div style={{ marginBottom: 12, padding: '8px 12px', background: '#FCEBEB', border: '1px solid #F5C2C2', borderRadius: 6, color: '#A32D2D', fontSize: 12 }}>{error}</div>}

        {/* Placeholder reference toggle */}
        <button onClick={() => setShowPlaceholderRef(s => !s)}
          style={{ fontSize: 12, padding: '6px 12px', border: '0.5px solid rgba(0,0,0,0.15)', borderRadius: 6, background: showPlaceholderRef ? '#FDF0EB' : '#FFFFFF', color: showPlaceholderRef ? '#E8734A' : '#6B6B6B', cursor: 'pointer', marginBottom: 12, fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}>
    ...
    <div style={{ padding: '20px 24px', overflow: 'auto', height: '100%', background: '#FAF9F7' }}>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 12, padding: 12, overflowX: 'auto' }}>
        <table style={{ borderCollapse: 'separate', borderSpacing: 4, fontSize: 11, fontFamily: "'IBM Plex Sans', system-ui, sans-serif" }}>
          <thead>
            <tr>
              <th style={{ textAlign: 'left', padding: '4px 8px', color: '#9E9E9E', fontWeight: 500, fontSize: 10, textTransform: 'uppercase' }}>Seniority \\ Situation</th>
              {situations.map(sit => (
                <th key={sit} style={{ padding: '4px 6px', color: '#6B6B6B', fontWeight: 500, fontSize: 10, textAlign: 'center', minWidth: 70, position: 'relative' }}>
          ...
          <tbody>
            {seniorities.map(sen => (
              <tr key={sen}>
                <td style={{ padding: '4px 8px', color: '#1A1A1A', fontWeight: 500, fontSize: 11, whiteSpace: 'nowrap' }}>
                  {sen.replace(/_/g, ' ')}
                </td>
                {situations.map(sit => {
