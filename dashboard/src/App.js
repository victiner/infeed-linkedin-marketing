...
const PLACEHOLDER_CATALOG = [
  {
    ...
    tokens: [
      ...
      { token: '{lead.firm}',        desc: 'Current company',                  example: 'Morgan Stanley' },
      { token: '{lead.role}',        desc: 'Current role',                     example: 'Analyst' },
      { token: '{lead.location}',    desc: 'Current location',                 example: 'London' },
      { token: '{lead.seniority}',   desc: 'Detected seniority',               example: 'analyst' },
    ],
  },
  {
    namespace: 'notes', label: 'Conversation notes (extracted by Haiku)', color: '#8B4513', bg: '#FAEEDA',
    tokens: [
      { token: '{notes.interest_track}',     desc: 'Industry they\'re targeting',           example: 'AM' },
...
function VoiceDnaPanel() {
  ...
  const insertPlaceholder = (token) => {
    ...
    if (!ta || !document.body.contains(ta)) {
      // No live textarea — fall back to clipboard
    ...
    // Update the right state slice based on the textarea's data-bind attribute
    ...
    if (stateName === 'editing' && field) {
      ...
      // Unknown binding — fall back to clipboard so user can paste manually
      try { navigator.clipboard.writeText(token); } catch {}
      return 'copied';
    }
    // Restore cursor position after the React re-render
    setTimeout(() => {
      ...
      {sortedKeys.map(key => {
        ...
        return (
          <div key={key} style={{ background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, padding: 12, marginBottom: 8 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: '#1A1A1A' }}>{key.replace(/_/g, ' ')}</div>
              <button onClick={() => removeKey(activeAxis, key)} disabled={busy}
                style={{ fontSize: 11, padding: '3px 10px', border: '0.5px solid #F5C2C2', borderRadius: 5, background: '#FFFFFF', color: '#A32D2D', cursor: busy ? 'wait' : 'pointer' }}>
                Remove key
              </button>
            </div>
