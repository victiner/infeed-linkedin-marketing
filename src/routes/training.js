...
// opts (all optional, all back-compat):
//   suppressSilent — disallow the SILENT outcome entirely. Set when the auto-fill
//     flow is targeting a non-ghost situation: the whole point is to land a real
//     lead message that fits that situation, so we cannot let the simulator
//     short-circuit into "no reply" → follow-up flow → wrong cell.
//   targetSituation — when present, the simulator is told what KIND of reply to
//     produce so the next-turn classifier lands in the target cell.
async function simulateLeadReply(lead, thread, opts = {}) {
  ...
  const { suppressSilent = false, targetSituation = null } = opts;

  // Per-situation guidance for the REPLY case so the next-turn classifier lands
  // in the target cell (mirrors the situationSteer map in synthesizeStarterThread).
  const situationGuidance = targetSituation ? ({
    neutral:                   'Reply must be a flat, short non-committal acknowledgement ("ok", "go on", "what is this") — no objection, no buying signal.',
    curious:                   'Reply must be a clear question about what we do, how it works, what kinds of roles ("how does this work", "what kind of roles", "what\'s on the list").',
    price_objection:           'Reply must be a clear price/cost objection ("how much", "is it free", "sounds expensive", "can\'t pay", "is there a free version"). Short.',
    time_objection:            'Reply must be a clear time/bandwidth objection ("super busy", "no time", "exam period", "maybe later", "ping me later").',
    confidentiality_objection: 'Reply must be a clear confidentiality concern ("would my firm see this", "is this confidential", "can\'t have my company find out", "discreet?").',
    has_alternative:           'Reply must signal you already have a similar source ("I have a network", "already use X", "covered thanks", "have an in").',
    buying_signal:             'Reply must be explicit buying intent ("send me the list", "let\'s do it", "how do I start", "send the link", "sign me up").',
    frustrated:                'Reply must be annoyed / hostile ("stop messaging", "this is spam", "why are you DMing me", "leave me alone").',
    wants_intro_to_specific_firm: 'Reply must name a SPECIFIC firm ("any roles at Goldman?", "do you cover Citadel?", "introductions at PJT?").',
    follow_up_after_ghosting:  '', // ghost flow handled separately
  })[targetSituation] || '' : '';
...
${situationGuidance ? `\nTARGET SITUATION (your reply MUST match this — this is the whole point of the scenario):\n${situationGuidance}\n` : ''}
${suppressSilent
  ? `TWO POSSIBLE OUTCOMES — pick one. SILENT is NOT an option in this scenario:
...
1. REPLY ({ "reply": "...", "ended": false }) — you respond.
   ...
   - Tone: ${targetSituation ? 'matches the TARGET SITUATION above (skeptical/curious/objecting/etc. as specified).' : 'skeptical, guarded, busy, sometimes dismissive. Not hostile — just realistic.'}
   - Never thank, never apologize, never be enthusiastic (unless target is buying_signal).
   - This is a TRAINING simulation for a specific situation cell — you MUST produce a reply that lets the user practice that situation. Going silent would skip the cell entirely.

2. ENDED ({ "reply": "not interested" / similar, "ended": true }) — you reject and stop.
   - Use ONLY for clear rejection ("not interested", "stop messaging", explicit no) or explicit agreement ("send the link").
   - Don't use ENDED just to avoid a hard situation — REPLY in-character instead.

Return ONLY valid JSON, no markdown:
{ "reply": "...", "ended": false }   (option 1 — preferred)
{ "reply": "...", "ended": true }    (option 2 — only for genuine rejection / agreement)`
...
{ "reply": "...", "ended": true }    (option 3)`}`;
  ...
  try {
    ...
    if (parsed.silent === true) {
      // If the caller asked us to suppress silent (auto-fill targeting a non-ghost
      // situation), the model still chose silent — coerce to a generic skeptical
      // reply so the cell actually gets trained. Better to have a slightly-off
      // reply than to skip the cell entirely.
      if (suppressSilent) {
        const fallback = targetSituation === 'curious' ? 'how does this work'
          : targetSituation === 'price_objection' ? 'how much'
          : targetSituation === 'time_objection' ? 'no time right now'
          : targetSituation === 'confidentiality_objection' ? 'would my firm see this'
          : targetSituation === 'has_alternative' ? 'already have a network'
          : targetSituation === 'buying_signal' ? 'how do I sign up'
          : targetSituation === 'frustrated' ? 'stop messaging me'
          : targetSituation === 'wants_intro_to_specific_firm' ? 'any roles at Goldman?'
          : 'go on';
        console.log(`[SimLeadReply] suppressSilent active, model chose silent — coerced to fallback for ${targetSituation || 'no-target'}`);
        return { reply: fallback, ended: false };
      }
      return { reply: null, silent: true, ended: false };
    }
    return parsed;
  } catch {
...
router.post('/simulation/step', async (req, res) => {
  ...
  try {
    ...
    // When auto-fill is targeting a non-ghost situation, suppress the lead's
    // SILENT outcome — the whole point of the run is to land an in-situation
    // reply. Without this, Haiku still picks silent ~30% of the time and the
    // dashboard pops the follow-up nudge, sending the save into the wrong cell.
    const leadOpts = (targetSituation && targetSituation !== 'follow_up_after_ghosting')
      ? { suppressSilent: true, targetSituation }
      : {};
    const leadOutcome = await simulateLeadReply(lead, threadWithOurs, leadOpts);
...
router.post('/simulation/lead-reply', async (req, res) => {
  const { lead, thread, targetAvatarId } = req.body || {};
  if (!lead) return res.status(400).json({ error: 'lead required' });
  const safeThread = Array.isArray(thread) ? thread : [];
  ...
  try {
    // Mirror /simulation/step: when targeting a non-ghost situation, suppress
    // the simulator's silent outcome and the deterministic force-silent override.
    const targetParsed = targetAvatarId ? avatars.parseAvatarId(targetAvatarId) : null;
    const targetSituation = targetParsed?.situation || null;
    const allowGhost = !targetSituation || targetSituation === 'follow_up_after_ghosting';
    const leadOpts = (targetSituation && !allowGhost)
      ? { suppressSilent: true, targetSituation }
      : {};
    const leadOutcome = await simulateLeadReply(lead, safeThread, leadOpts);
    const ourMsgCount = safeThread.filter(m => m.sender === 'us').length;
    const ghostProbability = !allowGhost ? 0
      : ourMsgCount >= 3 ? 0.5
      : ourMsgCount >= 2 ? 0.4
      : 0;
    const forceSilent = !leadOutcome.ended && Math.random() < ghostProbability;
