...
function buildSimGuidance(currentCtx) {
  ...
  let corrections = topByType('correction', 5).filter(p => p.original);
  ...
  if (totalMatched === 0) {
    usedFallback = true;
    corrections = topByType('correction', 5, false).filter(p => p.original);
  ...
  if (corrections.length) {
    blocks.push(`\nCORRECTIONS (never write like the BAD, always like the GOOD) — each shows the conversation that preceded the correction:\n${corrections.map(p => {
  ...
  if (annotations.length) {
    blocks.push(`\nPHRASE-LEVEL FEEDBACK (from highlighted text — feedback comments refer to the exchange shown):\n${annotations.map(p => {
      const ctx = renderThread(p.thread);
