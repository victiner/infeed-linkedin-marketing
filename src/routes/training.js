...
// Session-level tracking of which leads have been used in conversations
const usedConversationLeads = new Set();

// Pick a scenario that's under the saturation threshold.
// Returns null when all scenarios are saturated — callers should auto-generate.
function pickUnsaturatedScenario(excludeLeads = null) {
  ...
  // Exclude leads already used in this session's conversations
  if (excludeLeads && excludeLeads.size > 0) {
    candidates = candidates.filter(s => !excludeLeads.has(s.lead?.name));
    if (candidates.length === 0) candidates = scenarios;
  }

  const withCounts = candidates.map(s => ({
    scenario: s,
  ...
  // Everything saturated — return null so callers trigger auto-generation
  return null;
}

// Analyze which seniority/stage combos need more training
function getCoverageGaps() {
  ...
  // Build a matrix of seniority × stage → total training count
  const matrix = {};
  ...
  scenarios.forEach(s => {
    const sen = s.seniority || 'unknown';
    const stg = s.stage || 'unknown';
    if (matrix[sen] && matrix[sen][stg] !== undefined) {
      matrix[sen][stg] += store.getScenarioCount(wsId, s);
    }
  });

  // Rank combos by training count (lowest first)
  const ranked = [];
  for (const sen of seniorities) {
    for (const stg of stages) {
      ranked.push({ seniority: sen, stage: stg, count: matrix[sen][stg] });
    }
  }
  ranked.sort((a, b) => a.count - b.count);
  return { ranked, seniorities, stages };
}

// Generate new draft scenarios — prioritizes weakest seniority/stage combos
async function generateDraftScenarios() {
  const ws = workspace.get();
  const existing = getDraftScenarios();
  const existingLeads = existing.map(s => s.lead?.name).filter(Boolean);
  const { ranked, seniorities, stages } = getCoverageGaps();

  // Pick the 5 weakest combos to generate for
  const weakest = ranked.slice(0, 5);
  const gapDescription = weakest
    .map(g => `- ${g.seniority} / ${g.stage} (${g.count} trained so far)`)
    .join('\n');

  const pastPrefs = store.getTrainingPreferences(workspace.getId())
    .filter(p => p.type === 'draft')
    .slice(-15)
    .map(p => `Stage: ${p.scenario?.stage || '?'}, Seniority: ${p.scenario?.seniority || '?'}, Chosen: "${p.chosen}"`)
    .join('\n');

  const prompt = `Generate 5 new LinkedIn DM training scenarios for ${ws.company?.name || 'a high-finance career platform'}. Each scenario simulates a lead at a specific funnel stage.

The platform: ${ws.company?.what_it_is || 'curated high-finance roles + application support'}

PRIORITY: Generate scenarios for these UNDERCOVERED combos first (least trained):
${gapDescription}

Available seniority levels: ${seniorities.join(', ')}
Available stages: ${stages.join(', ')}

DO NOT reuse these lead names: ${existingLeads.slice(-30).join(', ')}
...
Make leads realistic — real firm names, realistic roles, varied geographies.

${pastPrefs ? `STYLE CONTEXT (match this tone in the "context" field):\n${pastPrefs}\n` : ''}
...
[{"stage": "...", "seniority": "...", "lead": {"name": "...", "role": "...", "company": "...", "location": "...", "background": "..."}, "lastMessage": "...", "context": "...", "sentiment": "..."}, ...]`;
  ...
  try {
    ...
    const scenarios = match ? JSON.parse(match[0]) : [];
    if (!ws.training_scenarios) ws.training_scenarios = [];
    scenarios.forEach(s => {
      if (s.stage && s.lead?.name) {
        ws.training_scenarios.push(s);
        console.log(`[Training] Generated draft scenario: ${s.lead.name} (${s.seniority}/${s.stage})`);
      }
    });
...
// Sentiment scenarios — seed pool, auto-expanded when exhausted
...
// Routing scenarios — seed pool, auto-expanded when exhausted
...
// Track how many times each scenario has been shown per workspace (max 2)
const MAX_SHOWS = 2;
const showCounts = new Map(); // Map<wsId, Map<scenarioKey, count>>

function getShowCount(wsId, type, key) {
  const wsMap = showCounts.get(wsId);
  if (!wsMap) return 0;
  return wsMap.get(`${type}:${key}`) || 0;
}

function incrementShowCount(wsId, type, key) {
  if (!showCounts.has(wsId)) showCounts.set(wsId, new Map());
  const wsMap = showCounts.get(wsId);
  const full = `${type}:${key}`;
  wsMap.set(full, (wsMap.get(full) || 0) + 1);
}

function sentimentKey(scenario) { return scenario.message; }
function routingKey(scenario) {
  return (scenario.messages || []).map(m => m.text).join('|');
}

// Pick from a pool, excluding scenarios shown >= MAX_SHOWS times
function pickFromPool(pool, type, keyFn) {
  const wsId = workspace.getId();
  const available = pool.filter(s => getShowCount(wsId, type, keyFn(s)) < MAX_SHOWS);
  if (available.length > 0) {
    const picked = available[Math.floor(Math.random() * available.length)];
    incrementShowCount(wsId, type, keyFn(picked));
    return picked;
  }
  return null;
}

// Generate new sentiment scenarios via Claude when the seed pool is exhausted
async function generateSentimentScenarios() {
  const wsId = workspace.getId();
  const existingMessages = SENTIMENT_SCENARIOS.map(s => s.message);

  // Count how many times each sentiment type has been trained
  const sentimentTypes = ['positive', 'neutral', 'negative', 'frustrated', 'question', 'buying_signal'];
  const sentimentCounts = {};
  sentimentTypes.forEach(t => { sentimentCounts[t] = 0; });
  store.getTrainingPreferences(wsId)
    .filter(p => p.type === 'sentiment')
    .forEach(p => { sentimentCounts[p.chosen] = (sentimentCounts[p.chosen] || 0) + 1; });

  const weakest = sentimentTypes
    .map(t => ({ type: t, count: sentimentCounts[t] }))
  ...
  const pastPrefs = store.getTrainingPreferences(wsId)
    .filter(p => p.type === 'sentiment')
    .slice(-10)
    .map(p => `Message: "${p.scenario?.message || '?'}" → User classified as: ${p.chosen}`)
    .join('\n');

  const prompt = `Generate 5 new realistic LinkedIn DM messages that a lead might send, for sentiment classification training.

These messages should feel like real LinkedIn replies in the context of a high-finance career intelligence platform reaching out to finance professionals about curated job opportunities.

PRIORITY — generate more messages for these UNDERREPRESENTED sentiment types:
${gapDescription}

Sentiment categories: positive, neutral, negative, frustrated, question, buying_signal

DO NOT repeat or closely paraphrase these existing messages:
${existingMessages.slice(-20).map(m => `- "${m}"`).join('\n')}

${pastPrefs ? `Recent training context:\n${pastPrefs}\n` : ''}
...
[{"message": "...", "expected": "positive|neutral|negative|frustrated|question|buying_signal"}, ...]`;
  ...
  try {
    ...
    const scenarios = match ? JSON.parse(match[0]) : [];
    scenarios.forEach(s => {
      if (s.message && s.expected) {
        SENTIMENT_SCENARIOS.push(s);
        console.log(`[Training] Generated sentiment scenario: "${s.message}" (${s.expected})`);
      }
    });
    return scenarios.length;
  } catch { return 0; }
...
// Generate new routing scenarios via Claude when the seed pool is exhausted
async function generateRoutingScenarios() {
  const wsId = workspace.getId();
  const existingKeys = ROUTING_SCENARIOS.map(s => routingKey(s));
  ...
  store.getTrainingPreferences(wsId)
    ...
    .forEach(p => { routeCounts[p.chosen] = (routeCounts[p.chosen] || 0) + 1; });

  const weakest = routes
    .map(r => ({ route: r, count: routeCounts[r] }))
    .sort((a, b) => a.count - b.count);
  ...
  const pastPrefs = store.getTrainingPreferences(wsId)
    .filter(p => p.type === 'routing')
    .slice(-10)
    .map(p => `Scenario: "${p.scenario?.messages?.map(m => m.text).join(' → ') || '?'}" → User chose: ${p.chosen}`)
    .join('\n');

  const prompt = `Generate 5 new LinkedIn DM routing scenarios for training. Each scenario has a lead profile and a short 2-message conversation (one from us, one reply from them). The user must decide what action to take next.

Context: This is for ${ws.company?.name || 'a'} — ${ws.company?.tagline || 'a career platform'}. We reach out to finance professionals about curated job opportunities and application support.

PRIORITY — generate scenarios where the correct answer is one of these UNDERREPRESENTED routes:
${gapDescription}

Available routing actions: ${routes.join(', ')}

Cover a variety of stages (cold, warm, hot), seniorities, and sentiments. Make conversations realistic — short DM style, real firm names.

DO NOT duplicate these existing conversation snippets:
${existingKeys.slice(-10).map(k => `- "${k}"`).join('\n')}

${pastPrefs ? `Recent routing choices:\n${pastPrefs}\n` : ''}
...
[{"lead": {"name": "...", "role": "...", "stage": "cold|warm|hot"}, "messages": [{"sender": "us", "text": "..."}, {"sender": "them", "text": "..."}], "expectedRoute": "..."}, ...]`;
  ...
  try {
    ...
    const scenarios = match ? JSON.parse(match[0]) : [];
    scenarios.forEach(s => {
      if (s.lead && s.messages && s.expectedRoute) {
        ROUTING_SCENARIOS.push(s);
        console.log(`[Training] Generated routing scenario: ${s.lead.name} → ${s.expectedRoute}`);
      }
    });
    return scenarios.length;
  } catch { return 0; }
...
router.get('/scenario', async (req, res) => {
  ...
  try {
    if (type === 'sentiment') {
      let scenario = pickFromPool(SENTIMENT_SCENARIOS, 'sentiment', sentimentKey);
      if (!scenario) {
        await generateSentimentScenarios();
        scenario = pickFromPool(SENTIMENT_SCENARIOS, 'sentiment', sentimentKey);
      }
      if (!scenario) {
        scenario = SENTIMENT_SCENARIOS[Math.floor(Math.random() * SENTIMENT_SCENARIOS.length)];
      }
    ...
    if (type === 'routing') {
      let scenario = pickFromPool(ROUTING_SCENARIOS, 'routing', routingKey);
      if (!scenario) {
        await generateRoutingScenarios();
        scenario = pickFromPool(ROUTING_SCENARIOS, 'routing', routingKey);
      }
      if (!scenario) {
        scenario = ROUTING_SCENARIOS[Math.floor(Math.random() * ROUTING_SCENARIOS.length)];
      }
    ...
    // Draft scenario — pick the least-trained one, auto-generate if all saturated
    let scenario = pickUnsaturatedScenario();
    if (!scenario) {
      await generateDraftScenarios();
      scenario = pickUnsaturatedScenario();
    }
    if (!scenario) {
      // Fallback: pick any scenario at random
      const all = getDraftScenarios();
      scenario = all.length ? all[Math.floor(Math.random() * all.length)] : null;
    }
    if (!scenario) {
...
router.get('/stats', (req, res) => {
  ...
  prefs.forEach(p => {
    // Count by source for conversation/targeted, by type for others
...
// POST /training/conversation/start — coverage-driven, avoids repeating leads
router.post('/conversation/start', async (req, res) => {
  try {
    // Try to pick a scenario with a lead not yet used this session
    let scenario = pickUnsaturatedScenario(usedConversationLeads);
    if (!scenario) {
      await generateDraftScenarios();
      scenario = pickUnsaturatedScenario(usedConversationLeads);
    }
    if (!scenario) {
      // All exhausted even after generation — clear session tracking and retry
      usedConversationLeads.clear();
      scenario = pickUnsaturatedScenario();
    }
    if (!scenario) {
      const all = getDraftScenarios();
    ...
    // Track this lead so it won't repeat next conversation
    if (scenario.lead?.name) usedConversationLeads.add(scenario.lead.name);
...
router.get('/targeted', async (req, res) => {
  ...
  try {
    ...
    if (filtered.length === 0) {
      // No scenarios for this filter — generate new ones
      await generateDraftScenarios();
      const newScenarios = getDraftScenarios();
      filtered = newScenarios;
      if (seniority) filtered = filtered.filter(s => s.seniority === seniority);
      if (stage) filtered = filtered.filter(s => s.stage === stage);
      if (filtered.length === 0) {
    ...
    // Pick least-trained among filtered, auto-generate if all saturated
    const wsId = workspace.getId();
    const withCounts = filtered.map(s => ({ scenario: s, count: store.getScenarioCount(wsId, s) }));
    const allSaturated = withCounts.every(x => x.count >= SATURATION_THRESHOLD);
    if (allSaturated) {
      await generateDraftScenarios();
      const refreshed = getDraftScenarios().filter(s =>
        (!seniority || s.seniority === seniority) && (!stage || s.stage === stage)
      );
      refreshed.forEach(s => {
        if (!withCounts.find(x => x.scenario === s)) {
          withCounts.push({ scenario: s, count: store.getScenarioCount(wsId, s) });
        }
      });
    }
