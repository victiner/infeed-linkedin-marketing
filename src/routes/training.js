...
router.post('/preference', (req, res) => {
  const { type, scenario, chosen, original, optionIndex, isCustom, source, thread } = req.body;
...
router.post('/annotate', (req, res) => {
  const { scenario, optionText, selectedText, feedback, rating, source, thread } = req.body;
...
function buildSimGuidance(currentCtx) {
  ...
  // Fallback: if NOTHING matched the context, fall back to most-recent preferences
  // so every simulation reflects the user's voice even when their trained scenarios
  // don't overlap with this simulated lead.
  const totalMatched = corrections.length + styleOnly.length + thumbsUp.length + thumbsDown.length + annotations.length;
  ...
  // Keep only the last few messages of each stored thread — enough to give context,
  // not enough to blow past the token budget.
  const THREAD_TAIL = 4;
  const renderThread = (thread) => {
    if (!Array.isArray(thread) || thread.length === 0) return null;
    return thread.slice(-THREAD_TAIL)
...
- MAXIMUM 12 words per reply. Most replies 3-8 words. One-liners, fragments, no punctuation sometimes.
- Tone: skeptical, guarded, busy, sometimes dismissive or slightly annoyed. Not hostile — just realistic.
- Never thank, never apologize, never be enthusiastic. No "that's interesting!", no "appreciate the message".
...
- If they're being pitched → be resistant. If asked for details → reluctant to share. If asked to commit → most leads hesitate.
- Roughly 60% of replies should be negative / skeptical / brief. 30% neutral-curious. 10% warm.
- Realistic drop-off: after 2-3 messages, many leads go silent or say "not interested" — if the pitch is weak or pushy, end the conversation.

END CONVERSATION (set ended: true) when:
- You'd realistically stop replying (most common — pitch wasn't compelling, too many messages, got bored)
- You're clearly rejecting them ("not interested", "stop messaging")
- You've actually agreed (rare — this should only happen if the outreach was genuinely compelling)

Return ONLY valid JSON, no markdown:
{ "reply": "the lead's short skeptical message", "ended": false }`;
...
router.post('/simulation/step', async (req, res) => {
  ...
  try {
    ...
    console.log(`[Simulation] stage=${stage} route=${routingDecision.routing_decision} asset=${selectedAsset?.id || 'none'} seniority=${leadProfile.seniority} — pulled: corrections=${guidance.counts.corrections} style=${guidance.counts.style} thumbs+=${guidance.counts.thumbsUp} thumbs-=${guidance.counts.thumbsDown} annotations=${guidance.counts.annotations} fallback=${guidance.usedFallback} (of ${guidance.counts.totalPrefs} total)`);
    const ourMessage = (await claude.draftMessage(safeThread, leadProfile, routingDecision, selectedAsset, guidance.text)).trim();
...
router.post('/simulation/ghost', async (req, res) => {
  ...
  try {
    ...
    const guidance = buildSimGuidance(currentCtx);
    console.log(`[Simulation/Ghost] seniority=${leadProfile.seniority} situation=follow_up_after_ghosting — pulled: corrections=${guidance.counts.corrections} style=${guidance.counts.style} thumbs+=${guidance.counts.thumbsUp} thumbs-=${guidance.counts.thumbsDown}`);
