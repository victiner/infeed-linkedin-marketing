...
router.post('/preference', async (req, res) => {
  const { type, scenario, chosen, original, optionIndex, isCustom, source, thread, avatar } = req.body;
...
async function synthesizeStarterThread({ scenario, leadProfile, targetStage }) {
...
- Our messages reflect a high-finance career platform — student-built, curated unadvertised roles. NEVER mention pricing.
- Lead replies are 3-12 words, fragments allowed.
...
// body: { seniority?, stage? } — picks or generates a matching scenario, and
// for non-cold-opener stages also synthesizes the prior thread so the next
// turn's classifyAvatar lands in the correct matrix cell.
router.post('/simulation/start', async (req, res) => {
  ...
  try {
    ...
    const starterThread = await synthesizeStarterThread({ scenario, leadProfile, targetStage });
...
router.post('/simulation/step', async (req, res) => {
  ...
  try {
    ...
    if (forceSilent) {
      console.log(`[Simulation] Force-silent triggered (our msg #${ourMsgCount}, p=${ghostProbability}) — model returned ${leadOutcome.silent ? 'silent' : 'reply'}`);
    }
