...
router.get('/scenario', async (req, res) => {
  ...
  try {
    ...
    // Draft scenario — pick the least-trained one first, auto-generate if all saturated
    let scenario = pickUnsaturatedScenario();
    const wsId = workspace.getId();
    const allSaturated = scenario && getDraftScenarios().every(s => store.getScenarioCount(wsId, s) >= SATURATION_THRESHOLD);
    if (!scenario || allSaturated) {
