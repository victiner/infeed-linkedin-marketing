...
router.get('/coverage', (req, res) => {
  initQueues();
  const remaining = {
    draft: queueSize('draft'),
    conversation: queueSize('conversation'),
    sentiment: queueSize('sentiment'),
    routing: queueSize('routing'),
  };
  const total = Object.values(remaining).reduce((a, b) => a + b, 0);
  const prefs = store.getTrainingPreferences(workspace.getId());

  res.json({
    total: getDraftScenarios().length,
    trained: prefs.length,
    remaining,
    totalRemaining: total,
    percentComplete: total > 0 ? Math.round(((getDraftScenarios().length - remaining.draft) / getDraftScenarios().length) * 100) : 100,
  });
});
...
router.get('/targeted', async (req, res) => {
  ...
  try {
    ...
    if (idx >= 0) {
      ...
      // Generate specifically for this filter
      const batch = await generateDraftBatch();
      const matching = batch.filter(s =>
        (!seniority || s.seniority === seniority) && (!stage || s.stage === stage)
      );
      const nonMatching = batch.filter(s =>
        !(!seniority || s.seniority === seniority) || !(!stage || s.stage === stage)
      );
      enqueue('draft', nonMatching);
      scenario = matching[0];
      if (matching.length > 1) enqueue('draft', matching.slice(1));
    }
