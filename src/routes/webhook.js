...
router.post('/heyreach', async (req, res) => {
  ...
  const isInCampaign = !!(campaign?.id);

  if (isInCampaign) {
    console.log(`[Webhook] Campaign lead: ${campaign.name || campaign.id}`);
  } else {
    console.log(`[Webhook] Non-campaign lead — will process but block auto-send`);
  }

  // Tag the payload so the router knows whether to allow auto-send
  payload._isInCampaign = isInCampaign;
  ...
  setImmediate(async () => {
    try {
      ...
      console.log(`[Webhook] Processed: ${result.action} for ${result.lead?.name} (autoSent: ${result.autoSent}, campaign: ${isInCampaign})`);
    } catch (err) {
