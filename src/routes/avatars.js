...
// GET /api/avatars/coverage/scoped?seniority=analyst
//   Returns coverage filtered to one seniority row, plus the next gap cell
//   (lowest-coverage, breaking ties by emptiness then id). Powers the dashboard's
//   "Auto-fill analyst coverage" flow — the front-end calls this on each save
//   and jumps the user to the returned `next` cell.
router.get('/coverage/scoped', (req, res) => {
  const wsId = req.query.workspace || workspace.getId();
  const { seniority } = req.query;
  res.json({ workspace: wsId, ...avatars.getCoverageScoped(wsId, { seniority }) });
});
