...
router.patch('/example/:airtableId', async (req, res) => {
  try {
    const allowed = ['isCanonical', 'avatar', 'chosen', 'original', 'feedback', 'selectedText', 'rating'];
    const fields = {};
...
router.post('/:avatarId/examples', async (req, res) => {
  try {
    ...
    const { type, chosen, original = '', feedback = '', isCanonical = false } = req.body || {};
    const allowedTypes = new Set(['thumbs_up', 'thumbs_down', 'correction', 'draft']);
    ...
    res.json({ success: true, record: pref });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
