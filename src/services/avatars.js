...
function getCoverage(workspaceId) {
  const all = store.getTrainingPreferences(workspaceId);
  const buckets = new Map(); // avatarId -> { count, good, bad, canonical_good, canonical_bad }
  ...
  return {
    ...
    totals: {
      ...
      tagged:         all.filter(p => p.avatar).length,
      untagged:       all.filter(p => !p.avatar).length,
...
// Get all examples for a single avatar (used by the dashboard avatar browser)
function getAvatarExamples(workspaceId, avatarId) {
  return store.getTrainingPreferences(workspaceId)
    .filter(p => p.avatar === avatarId)
    .map(p => ({
    ...
    .sort((a, b) => (a.isCanonical === b.isCanonical ? 0 : a.isCanonical ? -1 : 1));
}
