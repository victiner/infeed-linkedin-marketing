...
function getCanonicalExamples(workspaceId, avatarId, opts = {}) {
  ...
  // Corrections are NOT counted as bad — the user's rewrite (chosen) is the
  // training signal we want; the original (BAD) side travels along inside the
  // good entry and is rendered as red context above the green chosen text.
  // Counting corrections as bad caused the same record to show up in both
  // lists, with `text = chosen` displayed under CANONICAL BAD — i.e. the user's
  // rewrite was being shown as something to avoid. Bug.
  const isBad = (p) => p.type === 'thumbs_down';
  ...
  const score = (p) => {
    ...
    // Simulation training is the explicit primary signal: the user deliberately
    // crafted the scenario, the rewrite, the rating, the highlight. Live records
    // are real feedback but often noisier (auto-sent, no explicit rating). Sim
    // wins by +5 on score and is auto-marked canonical for rewrites at save
    // time (see /training/preference).
    if (p.source === 'simulation') s += 5;
...
function getCoverage(workspaceId) {
  ...
  for (const av of listAllAvatars()) {
    buckets.set(av.id, {
      ...av,
      count: 0, good: 0, bad: 0, canonical_good: 0, canonical_bad: 0,
      // Quad-bucket counts that drive the "12 per cell" coverage standard.
      // Each bucket is independent — a 'correction' counts as a rewrite, NOT
      // also as good+bad, so the user has to provide separate thumbs_up /
      // thumbs_down examples to actually saturate the cell. Q&A pairs are
      // the lead-question + canonical-answer references the router pulls
      // when the inbound looks like a question.
      corrections: 0, thumbs_up: 0, thumbs_down: 0, qa: 0,
    });
  }
  for (const p of messageRecords) {
    ...
    // Corrections are NOT bucketed as bad — the rewrite (chosen) is the signal,
    // the original (BAD) side travels along on the good entry as context. See
    // the matching change in getCanonicalExamples() above.
    const isBad  = p.type === 'thumbs_down';
    if (isGood) b.good++;
    if (isBad)  b.bad++;
    if (p.isCanonical && isGood) b.canonical_good++;
    if (p.isCanonical && isBad)  b.canonical_bad++;
    if (p.type === 'correction' && p.chosen && p.original) b.corrections++;
    if (p.type === 'thumbs_up')   b.thumbs_up++;
    if (p.type === 'thumbs_down') b.thumbs_down++;
    if (p.type === 'qa' && p.question && p.chosen) b.qa++;
  }
  // Saturation: 3 corrections + 3 thumbs_down + 3 thumbs_up + 3 Q&A = 12 per cell.
  const TARGET = 3;
  for (const b of buckets.values()) {
    b.target_per_bucket = TARGET;
    b.needed_corrections = Math.max(0, TARGET - b.corrections);
    b.needed_bad         = Math.max(0, TARGET - b.thumbs_down);
    b.needed_good        = Math.max(0, TARGET - b.thumbs_up);
    b.needed_qa          = Math.max(0, TARGET - b.qa);
    b.deficit            = b.needed_corrections + b.needed_bad + b.needed_good + b.needed_qa;
    b.saturated          = b.deficit === 0;
  }
  ...
  return {
    ...
    totals: {
      ...
      cells_saturated: cells.filter(c => c.saturated).length,
      cells_total:    cells.length,
    },
  };
}

// Coverage filtered to a seniority row (or anything matching the predicate).
// Returns the same shape as getCoverage but with row-scoped totals + the
// next-gap pointer the dashboard's auto-fill flow consumes.
function getCoverageScoped(workspaceId, { seniority } = {}) {
  const full = getCoverage(workspaceId);
  let cells = full.cells;
  if (seniority) cells = cells.filter(c => c.seniority === seniority);
  const ordered = [...cells].sort((a, b) => {
    if (a.deficit !== b.deficit) return b.deficit - a.deficit; // largest gap first
    if (a.count   !== b.count)   return a.count - b.count;     // emptier first
    return a.id.localeCompare(b.id);                            // stable
  });
  const nextGap = ordered.find(c => !c.saturated) || null;
  const totals = {
    cells_total:     cells.length,
    cells_saturated: cells.filter(c => c.saturated).length,
    cells_filled:    cells.filter(c => c.count > 0).length,
    deficit_total:   cells.reduce((s, c) => s + c.deficit, 0),
  };
  return {
    seniority: seniority || null,
    target_per_bucket: full.target_per_bucket,
    totals,
    cells,
    next: nextGap,
  };
...
function getAvatarExamples(workspaceId, avatarId) {
  return store.getTrainingPreferences(workspaceId)
    ...
    .map(p => ({
      ...
      rating:       p.rating,
      question:     p.question,
      // Pass the conversation snippet through so the matrix detail view can
      // render the context this example was trained against. Cap at the last
      // 6 messages — enough context, doesn't blow up the JSON payload.
      thread:       Array.isArray(p.thread) ? p.thread.slice(-6) : [],
      isCanonical:  !!p.isCanonical,
