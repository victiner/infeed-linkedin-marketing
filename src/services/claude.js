...
// opts (all optional, all back-compat — production callers pass none):
//   temperature   — override default of 0.6 (sim flow uses 0.85 for variety)
//   avoidOpeners  — string[] of recent drafts in this cell to avoid repeating
async function draftMessage(thread, leadProfile, routingDecision, asset = null, templateGuidance = null, opts = {}) {
  const prompt = DRAFT_MESSAGE_PROMPT(thread, leadProfile, routingDecision, asset, templateGuidance, {
    avoidOpeners: opts.avoidOpeners || [],
  });
...
//
// opts (optional, all back-compat — existing callers pass none):
//   model        — override default Haiku (e.g. SONNET for prior-thread synthesis
//                  where voice fidelity matters and we inject canonical examples)
//   temperature  — override default 0.7
async function raw(prompt, opts = {}) {
...
module.exports = { classifyAndRoute, draftMessage, classifyInboundSentiment, raw, SONNET, HAIKU };
