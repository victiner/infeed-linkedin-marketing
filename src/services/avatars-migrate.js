...
async function runMigration(workspaceId, opts = {}) {
  ...
  // NOTE: do NOT require airtableId here. Records from training simulations / conversation /
  // targeted modes can end up in memory without an airtableId when Airtable's create
  // silently failed (rate limit / network blip). Those records still need to be tagged so
  // they show up in the matrix; we mutate them by reference and best-effort sync to
  // Airtable (skipped when there is no id).
  const candidates = all.filter(p => isMessageRecord(p) && (reclassify || !p.avatar));
  const status = {
    ...
    for (let i = 0; i < candidates.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          ...
          try {
            // Always mutate the in-memory record by reference so the matrix
            // reflects the tag immediately, regardless of whether the record
            // ever made it to Airtable. updateTrainingFields below would do the
            // same in-memory write, BUT it looks up by airtableId — for records
            // missing an id that find() either returns undefined or matches the
            // wrong row. Direct mutation is the only safe path here.
            rec.avatar = avatarId;
            if (rec.airtableId) {
              // Best-effort Airtable sync; failure here doesn't undo the tag.
