...
async function addTrainingPreference(pref) {
  ...
  if (b) {
    try {
      ...
      // means we can re-push later via forceResyncTrainingToAirtable().
      // But DO surface so the caller / dashboard can warn the user.
...
async function loadTraining() {
  ...
  // re-pushed to memory (and resynced via forceResyncTrainingToAirtable).
  ...
  if (replayed > 0) {
    console.log(`[Store] Replayed ${replayed} local-only training records (not yet in Airtable). Run forceResyncTrainingToAirtable() to push them.`);
  }
}
