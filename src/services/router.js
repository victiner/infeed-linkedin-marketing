...
async function processInboundMessage(webhookPayload) {
  ...
  const isInCampaign = webhookPayload._isInCampaign !== false;
  console.log(`[Router] Processing message from ${senderName} (${senderProfileUrl}) [campaign: ${isInCampaign}]`);
  ...
  // Tag lead with campaign status
  if (!lead.tags) lead.tags = [];
  const campaignTag = isInCampaign ? 'in-campaign' : 'outside-campaign';
  if (!lead.tags.includes(campaignTag)) {
    lead.tags = lead.tags.filter(t => t !== 'in-campaign' && t !== 'outside-campaign');
    lead.tags.push(campaignTag);
  }
  ...
  if (sentimentCheck.requires_human) {
    ...
    slack.notify({
      ...
      lead, reason: `Sentiment: ${sentimentCheck.sentiment}`,
      routing: 'human_takeover', conversationId,
    });
  ...
  const shouldAutoSend = isInCampaign && routingDecision.routing_decision !== 'human_takeover' && confidence >= AUTO_SEND_THRESHOLD;

  if (!isInCampaign) {
    console.log(`[Router] Non-campaign lead — auto-send blocked, queued for review: ${senderName}`);
  }
