...
async function processInboundMessage(webhookPayload) {
  ...
  if (step && !step.useAI && step.template) {
    ...
    let avatarId = null;
    // Captured for the dashboard's "what fed this draft?" panel — same data as
    // the prompt sees, but as a structured object so the UI can render it.
    ...
    try {
      ...
      // Q&A retrieval: if the lead's last inbound looks like a question, pull the closest
      // matching Q&A pairs from this avatar (or fallback). Pure JS keyword similarity, no LLM.
      ...
      if (looksLikeQuestion) {
        try {
          qaMatches = avatars.findRelevantQAs(workspace.getId(), avatarId, lastInbound, { limit: 2 });
          if (qaMatches.length) {
            console.log(`[Router] Q&A retrieved: ${qaMatches.length} match(es) for "${lastInbound.slice(0, 60)}…"`);
          }
        } catch (err) {
          console.warn('[Router] Q&A retrieval failed:', err.message);
        }
      }
    ...
    // Build the structured breakdown for the dashboard's "what fed this draft?"
    // panel. Mirror the buildSimGuidance shape so sim and live render with the
    // same component on the client.
    var liveGuidanceBreakdown = {
  ...
  const convoWithDraft = store.addDraftToConversation(conversationId, {
    ...
    routingDecision,
    asset: selectedAsset,
    guidanceBreakdown: typeof liveGuidanceBreakdown !== 'undefined' ? liveGuidanceBreakdown : null,
...
function scoreTrainingRelevance(pref, currentCtx) {
  ...
  // Simulation training is the explicit primary signal: deliberate, scenario-
  // specific, with full context. Multiply sim records by 2x in the relevance-
  // scored fallback pool so they outrank live records here too. (Canonical
  // matrix examples already win on score+position above this.)
  if (pref.source === 'simulation') score *= 2;
...
function buildLeadProfile(lead) {
  return {
    ...
    industry: inferSegment(lead.role, lead.company),  // lead's CURRENT industry — used by {lead.industry} placeholder
