...
async function pollOnce() {
  ...
  try {
    ...
    for (const convo of conversations) {
      ...
      const senderId = convo.linkedInAccountId || convo.senderId;
      const payload = {
      ...
      // Route to the correct workspace based on the sender
      const wsId = workspace.findBySenderId(senderId);
      if (!wsId) {
        console.warn(`[Poller] Unknown senderId "${senderId}" for ${payload.leadName} — no workspace claims it. Skipping.`);
        continue;
      }

      try {
        await workspace.runInContext(wsId, async () => {
          const result = await processInboundMessage(payload);
          console.log(`[Poller] Processed [${wsId}] ${payload.leadName}: ${result.action}`);
        });
      } catch (err) {
        console.error(`[Poller] Failed to process message from ${payload.leadName} [${wsId}]:`, err.message);
      }
