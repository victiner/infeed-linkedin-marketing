...
const DRAFT_MESSAGE_PROMPT = (thread, leadProfile, routingDecision, asset, templateGuidance = null, opts = {}) => {
  const avoidOpeners = Array.isArray(opts.avoidOpeners) ? opts.avoidOpeners.filter(Boolean) : [];
...
${templateGuidance}` : ''}
${avoidOpeners.length ? `\nAVOID THESE OPENERS — we've used them recently in this exact context. Pick a different angle, a different first word, a different sentence shape. Do NOT start your message with anything that resembles these:
${avoidOpeners.map(t => `  - "${t}"`).join('\n')}
` : ''}
