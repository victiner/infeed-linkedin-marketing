...
function InboxPanel({ onStats }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', overflow: 'hidden' }}>
      {/* Thread list */}
      <div style={{ width: 260, borderRight: '0.5px solid var(--color-border-tertiary)', overflowY: 'auto', flexShrink: 0 }}>
        ...
        {conversations.map(c => {
          ...
          return (
            <div key={c.id} onClick={() => selectConvo(c)}
              style={{
                padding: '11px 14px', borderBottom: '0.5px solid var(--color-border-tertiary)',
                cursor: 'pointer', background: isSelected ? 'var(--color-background-secondary)' : 'transparent'
              }}>
          ...
          <>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {(sel.messages || []).map((m, i) => (
                <div key={i} style={{ maxWidth: '80%', alignSelf: m.sender === 'us' ? 'flex-end' : 'flex-start' }}>
                  <div style={{
                    padding: '8px 12px', borderRadius: 8, fontSize: 13, lineHeight: 1.5,
                    background: m.sender === 'us' ? '#E8734A' : 'var(--color-background-secondary)',
                    color: m.sender === 'us' ? '#fff' : 'var(--color-text-primary)',
                    border: m.sender === 'us' ? 'none' : '0.5px solid var(--color-border-tertiary)'
                  }}>{m.text}</div>
                  <div style={{ fontSize: 11, color: 'var(--color-text-tertiary)', marginTop: 3, padding: '0 4px', textAlign: m.sender === 'us' ? 'right' : 'left' }}>
...
export default function App() {
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: "'IBM Plex Sans', system-ui, sans-serif", background: '#FAF9F7', overflow: 'hidden' }}>
      ...
      {/* Sidebar */}
      <div style={{ width: 220, background: '#FFFFFF', borderRight: '1px solid var(--if-border, rgba(0,0,0,0.08))', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '16px 16px 14px', borderBottom: '1px solid var(--if-border, rgba(0,0,0,0.08))', display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 32, height: 32, borderRadius: 8, background: '#E8734A', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 13, fontWeight: 600, flexShrink: 0 }}>iF</div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#1A1A1A' }}>inFeed</div>
            <div style={{ fontSize: 10, color: '#9E9E9E', letterSpacing: '0.04em' }}>Distribution</div>
          </div>
        </div>
        <nav style={{ flex: 1 }}>
          {NAV.map(n => (
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: '#FFFFFF' }}>
        <div style={{ padding: '14px 24px', borderBottom: '1px solid var(--if-border, rgba(0,0,0,0.08))', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h1 style={{ fontSize: 16, fontWeight: 600, color: '#1A1A1A', margin: 0 }}>{NAV.find(n => n.id === activePanel)?.label}</h1>
        </div>
