...
const DRAFT_MESSAGE_PROMPT = (thread, leadProfile, routingDecision, asset, templateGuidance = null) => {
  ...
  const stageInstructions = {
    cold_opener: `Cold opener only. Do NOT mention job lists, the service, pricing, or InFeed by name. 1-2 lines. Something specific about them + curiosity hook.`,

    natural_response: `Fast responder — take an extra beat. React to what they said, build rapport, qualify their interest. No links, no pitch yet. Match their energy. 1-2 lines.`,

    value_pitch: `They replied — come to the point. React to what they said, then deliver the job list AND introduce the service in one message. Frame it: curated roles for their background + we handle the full application (CV, cover letter, submission). Include the URL. Do NOT mention pricing yet — that's the close stage. 3-4 lines max.`,

    close: `They're interested or asked about pricing. One clear ask — call or payment link, never both. Be specific: ${recommendedPlan && planDetails[recommendedPlan] ? planDetails[recommendedPlan] : 'Professional Plus at €69/month is the most common starting point.'}. 1-2 lines.`,

    follow_up: `No reply to last message. One line only. No apology. No "just checking in". Reference something specific.`
  };
